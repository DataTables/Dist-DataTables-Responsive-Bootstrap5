...
var _display=DataTable.Responsive.display,_original=_display.modal,_modal=$('<div class="modal fade dtr-bs-modal" role="dialog"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div><div class="modal-body"/></div></div></div>'),modal,_bs=window.bootstrap;DataTable.Responsive.bootstrap=function(a){_bs=a};
_display.modal=function(a){modal||(modal=new _bs.Modal(_modal[0]));return function(c,b,e){if(!$.fn.modal)_original(c,b,e);else if(!b){if(a&&a.header){b=_modal.find("div.modal-header");var d=b.find("button").detach();b.empty().append('<h4 class="modal-title">'+a.header(c)+"</h4>").append(d)}_modal.find("div.modal-body").empty().append(e());_modal.appendTo("body").modal();modal.show()}}};
